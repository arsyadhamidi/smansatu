                                    ...
                                    <tr>
                                        ...
                                        <td class="d-flex">
                                            @if ($data->status == 'Diterima')
                                                <form action="{{ route('kepsek-pengajuan.updateproses', $data->id) }}"
                                                    method="POST">
                                                    @csrf
                                                    <button type="submit" class="btn btn-sm btn-success mx-2"
                                                        onclick="return confirm('Apakah anda yakin untuk disetujui data ini ?')">
                                                        <i class="fas fa-check"></i>
                                                    </button>
                                                </form>
                                                <form action="{{ route('kepsek-pengajuan.updatetolak', $data->id) }}"
                                                    method="POST">
                                                    @csrf
